...
public class Exercise4Test extends CompanyDomainForKata
{
    ...
    public void whoSuppliesSandwichToaster()
    {
        ...
        Predicate<Supplier> suppliesToaster = s->ArrayIterate.anySatisfy(s.getItemNames(), name->name.equals("sandwich toaster"));   // s->ArrayIterate.anySatisfy(s.getItemNames(), item->item.equals("sandwich toaster"));

        // Find one supplier that supplies toasters.
        Supplier toasterSupplier = ArrayIterate.detect(this.company.getSuppliers(), suppliesToaster);                         //ArrayIterate.collect(company.getSuppliers(), Supplier::new).getFirst();
    ...
    {
        List<Order> orders = this.company.getMostRecentCustomer().getOrders();
        MutableList<Order> filtered = ListIterate.select(orders, o->o.getValue()>2);
